...
RENDER_CALLER_SIG(default_render_caller){
    ...
    // NOTE(ruben): Token highlighting
    {
        // Operator color
        Theme_Color operator_color = {};
        operator_color.tag = Stag_Text_Cycle_4;
        get_theme_colors(app, &operator_color, 1);
        
        // Procedure color
        Theme_Color procedure_color = {};
        procedure_color.tag = Stag_Text_Cycle_3;
        get_theme_colors(app, &procedure_color, 1);
        
        
        Temp_Memory temp = begin_temp_memory(scratch);
        
        // Stage 1: Read the tokens to be used for indentation.
        Cpp_Token_Array tokens = buffer_get_all_tokens(app, scratch, &buffer);
        
        // Get a pointer to the beginning of records
        Highlight_Record *records = push_array(scratch, Highlight_Record, 0);
        
        for (int i = 0; i < tokens.count; ++i) { // All tokens
            Cpp_Token token = tokens.tokens[i];
            
            // Skip if we can't see the token
            int32_t start = token.start;
            int32_t end = token.start + token.size;
            
            if(start > on_screen_range.one_past_last || end < on_screen_range.first)continue;
            
            // Operators
            if(token.type >= CPP_TOKEN_OPERATORS_BEGIN &&  token.type <= CPP_TOKEN_OPERATORS_END) {
                Highlight_Record *record = push_array(scratch, Highlight_Record, 1);
                record->first = start;
                record->one_past_last = end;
                record->color = operator_color.color;
            } else if(token.type == CPP_TOKEN_IDENTIFIER
                      && ( i < (tokens.count - 1)
                          && tokens.tokens[i+1].type == CPP_TOKEN_PARENTHESE_OPEN
                          )) {
                Highlight_Record *record = push_array(scratch, Highlight_Record, 1);
                record->first = start;
                record->one_past_last = end;
                record->color = procedure_color.color;
            }
            
        }
        // Some pointer math to see how many records we have
        int32_t record_count = (int32_t)(push_array(scratch, Highlight_Record, 0) - records);
        push_array(scratch, Highlight_Record, 1);
        
        
        // If we have tokens to highlight
        if (record_count > 0){
            sort_highlight_record(records, 0, record_count);
            Temp_Memory marker_temp = begin_temp_memory(scratch);
            Marker *markers = push_array(scratch, Marker, 0);
